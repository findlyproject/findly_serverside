...
import { jobPostSchema } from "../../Utils/zodSchema";
...
export const applyToJob = async (
  ...
  const resumdocument = findUser?.resumePDF?.find(
    (item) => item.isDeleted === false
  );
  const resumVideoFile = findUser?.resumeVideo?.find(
    (item) => item.isDeleted === false
  );
...
export const applydeJobs = async (
  req: Request,
  res: Response
): Promise<void> => {
  const userId = req.user?.id;
  ...
  const findeapllyedJObs = await JobApplication.find({ userId })
    .populate({
      path: "jobId",
      populate: {
        path: "company",
...
export const similarjobs = async (
  req: Request,
  res: Response
): Promise<void> => {
