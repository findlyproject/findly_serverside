...
// ✅ Define Cloudinary Storage for Media Uploads
const storage = new CloudinaryStorage({
  cloudinary,
  params: async (_req, file) => {
    console.log(`Uploading: ${file.originalname} - Type: ${file.mimetype}`);

    return {
      folder: "posts/media", // ✅ Store in "posts/media" folder
...
// ✅ Export Middleware for Media Uploads
export const uploadMedia = upload.fields([{ name: "media", maxCount: 5 },]) // ✅ Accept up to 5 media files
